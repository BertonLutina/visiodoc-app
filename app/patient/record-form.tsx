...
import {
  ...
  type PickedFile,
} from '@/services/medicalRecordAttachments';
...
export default function RecordForm() {
  ...
  const { data: records, error: recordsError } = useAsync(() => getMedicalRecords(patientId), [patientId]);
  ...
  /**
   * Id de la ligne créée pendant CETTE session de formulaire. Sert de garde anti-doublon :
   * si la création réussit mais que l'upload de la pièce jointe échoue ensuite, un second
   * "Enregistrer" doit mettre à jour cette ligne, pas en insérer une seconde.
   */
  ...
  const onSave = async () => {
    ...
    setError(null);
    try {
      // Une ligne déjà existante (édition) ou déjà créée par une tentative précédente
      // (échec d'upload) se met à jour — jamais un second INSERT pour le même formulaire.
      let id = recordId ?? createdId ?? undefined;
      if (id) {
        await updateMedicalRecord(
          ...
          { doctorId, patientId, kind },
          {
            kind,
            title,
            // `null` (et non `undefined`) pour qu'un champ vidé par l'utilisateur soit
            // réellement effacé en base au lieu de conserver silencieusement l'ancienne valeur.
  ...
  const openAttachment = async (path: string) => {
    try {
      ...
      if (!url) return; // pas de chemin Storage (mode démo) : rien à ouvrir
      await Linking.openURL(url);
    } catch (e: any) {
      setError(e?.message ?? "Impossible d'ouvrir cette pièce jointe.");
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-bg" edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
        ...
        {(existing?.attachments ?? []).map((a) => (
          <Pressable key={a.path} onPress={() => openAttachment(a.path)} className="mb-1">
            <Text className="font-sans text-sm text-accent underline">📎 {a.name}</Text>
          </Pressable>
        ))}
